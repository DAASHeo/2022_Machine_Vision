...
def drawROI(img, corners):
    cpy = img.copy() #이미지 복사해서 원본유지.

    c1 = (192, 192, 255) #색상값 B, G, R
    c2 = (128, 128, 255)
...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        for i in range(4):
            if cv2.norm(srcQuad[i] - (x, y)) < 25:  # 원 안에 있어야 드래그 가능
                dragSrc[i] = True
                ptOld = (x, y)
                break
...
src_original = cv2.imread('ticket.jpg')

if src_original is None:
    print('Image open failed!')
    sys.exit()

src = cv2.resize(src_original, (450, 600))
...
cv2.destroyAllWindows()
    ...
    # srcQuad : 현재 코너 동그라미들의 위치

    #print = (x,y) #배열 출력
    #onMouse함수에서 param무시하기
