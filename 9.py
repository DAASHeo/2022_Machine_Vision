...
img[:] = (255) #ndarray 클래스 내부 함수 fill()로 행렬의 원소값을 255로 채운다.
# np.zeros np.ones, np.full 차이점 정확하게 이해할 것!
title = "WINDOW1" #title 변수에 'rect'라는 윈도우 이름 할당
...
cv2.rectangle(img, p1, p2, red, cv2.FILLED) #시작좌표 p1에서 종료좌표인 p2까지 빨간색으로 채워진 사각형을 그린다.
